...
void HandleTCPClient(int clientSocket)
{
	...
	printf("Enter HandleTCPClient, clientSocket: %d\n", clientSocket);
	...
	for (;;){
		...
		// Receive Filename
		if (recv(clientSocket, filename, sizeof(filename), 0) != sizeof(filename)){
			DieWithError("Receive Filename Fails");
		}

		if (strcmp (filename, "EXIT") == 0){
			printf("Exit HandleTCPClient, clientSocket: %d\n", clientSocket);
			break;
		}
		...
		while (getline(&line, &len, fp) > 0) {
			...
			packet.seq_num++;
			if (send(clientSocket, &packet, sizeof(packet), 0) != sizeof(packet)){
			...
			if (send(clientSocket, buffer, line_lenght + 1, 0) != line_lenght + 1){
